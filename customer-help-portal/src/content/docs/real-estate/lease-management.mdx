...
5. **Process final settlement**
   ...
   | Unpaid utilities | (Amount) |
   | Repair deductions | (Amount) |
   | **Total deductions** | **(Sum of above)** |
   | Deposit held | (Deposit amount) |
   | **Refund due to tenant** | **(Deposit minus deductions)** |
...
**Example:** If your current rent is 100 units and you apply a fixed escalation of 5 units per year, the new rent becomes 105 units.
...
2. In the **Escalation Notes** field, record the increase for audit purposes (e.g., "Annual escalation: +(amount)").
...
**Example:** If your current rent is 100 units and you apply a 10% annual escalation, the new rent becomes 110 units.
...
3. **Review the impact:**
   ...
   | Rent (from charge template) | (1-BR rate) | (2-BR rate) |
   | Garbage | (Same) | (Same) |
   | Water | (Same) | (Same) |

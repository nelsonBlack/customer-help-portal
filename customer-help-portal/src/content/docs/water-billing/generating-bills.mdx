...
| Tier | Unit range | Rate | Units in tier | Charge |
|---|---|---|---|---|
| Tier 1 | 0--10 | (Rate A) | 10 | (Rate A × 10) |
| Tier 2 | 11--20 | (Rate B) | 10 | (Rate B × 10) |
| Tier 3 | 21+ | (Rate C) | 12 | (Rate C × 12) |
| | | | **Total Water** | **(Sum of all tiers)** |
...
| Charge | Amount |
|---|---|
| Standing charge | (Your configured amount) |
| Meter rental | (Your configured amount) |
| **Total Service Charges** | **(Sum of service charges)** |
...
| **Fixed amount** | A flat fee is added for each overdue invoice |
...
| Line item | Amount |
|---|---|
| Water charges (32 units) | (Calculated from tariff) |
| Standing charge | (Your configured amount) |
| Meter rental | (Your configured amount) |
| Late payment penalty | (If applicable) |
| Credit (overpayment from last month) | (If applicable) |
...
<Tabs>
  <TabItem label="SMS">
    SMS is a common distribution method. Each customer receives an SMS with:
    - Billing period
    - Consumption (units)
    - Total amount due
    - Payment deadline
    ...
    **Example SMS:**
    ```
    [YourCompany] Bill for [Period].
    Meter: [Meter Number]
    Units: [Consumption]
    Amount Due: [Amount]
    Due by: [Deadline]
    Pay via [Payment Method]
...
|---|---|---|
| Water charges | 8 units consumed at Tier 1 rate | (Tier 1 rate × 8) |
| Standing charge (pro-rated) | Standing charge × (14/28) | (Half of standing charge) |
| Meter rental (pro-rated) | Meter rental × (14/28) | (Half of meter rental) |
| **Total** | | **(Sum of pro-rated charges)** |

...
description: Set up M-Pesa integration, record manual payments, handle reconciliation, manage credits, and troubleshoot common payment issues in Analog Water.
---

import { Steps, Tabs, TabItem } from '@astrojs/starlight/components';

Getting paid is the whole point. Analog Water supports both automatic M-Pesa reconciliation and manual payment recording, giving you flexibility to handle whatever payment methods your customers prefer. This guide covers the full payment lifecycle — from setup to reconciliation and troubleshooting.

## M-Pesa payment setup

M-Pesa is the dominant payment method for water utilities in Kenya and increasingly across East Africa. Analog Water integrates directly with Safaricom's M-Pesa API to automatically match incoming payments to customer accounts.
...
4. Configure the **Account Format** — this determines how the system matches payments to customers. Common formats:
   - **Account Number** — Customers enter their Analog Water account number as the M-Pesa account reference.
...
2. **Safaricom sends a callback** to Analog Water with the payment details: amount, phone number, account reference, and transaction ID.

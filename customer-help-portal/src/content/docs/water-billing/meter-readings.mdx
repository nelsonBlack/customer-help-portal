...
description: Record meter readings from the Analog Water dashboard — single entries, batch readings, anomaly handling, and integration with the EasyBill mobile app.
---
...
Every reading in Analog Water follows the same basic flow:
...
Analog Water automatically flags readings that appear unusual based on the customer's historical consumption patterns. This is the **anomaly detection** system.
...
For field-based reading capture, Analog Water integrates with the **EasyBill** mobile app. The app allows field agents to:
...
4. **Readings sync** to the Analog Water dashboard automatically when the device has connectivity. Offline readings are queued and uploaded when the agent is back online.
...
:::note
The EasyBill mobile app is available for Android devices. Contact your Analog Water account representative for setup instructions and agent licenses.
:::

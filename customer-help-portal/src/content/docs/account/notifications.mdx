...
Notifications keep your customers informed about their bills and payments, and alert your team about operational events like reading anomalies or overdue accounts. Analog Water supports **SMS** (via Africa's Talking) and **email** notifications.
...
SMS is the most reliable way to reach customers in East Africa, where many users have basic phones without email access. Analog Water integrates with [Africa's Talking](https://africastalking.com/) for SMS delivery.
...
<Steps>
  ...
  4. **Enter credentials in Analog Water.** Go to **Settings > Company Settings > Notifications > SMS** and enter:
...
Africa's Talking provides a sandbox environment for testing without consuming your SMS balance. In the Africa's Talking dashboard, switch to "Sandbox" mode, generate sandbox credentials, and use those in Analog Water temporarily. Switch back to production credentials when you are ready to go live.
:::

### Email notifications

Email notifications are handled by Analog Water's infrastructure. You do not need to configure an SMTP server.
...
Emails are sent from a `noreply@analogwater.com` address with your configured "From name" in the display field.
...
| **Domain blocking** | Some corporate email servers block unknown senders. Ask the recipient's IT team to whitelist `analogwater.com`. |

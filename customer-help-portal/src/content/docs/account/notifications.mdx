...
SMS is a reliable way to reach customers. EasyBiller integrates with [Africa's Talking](https://africastalking.com/) for SMS delivery.
...
| `{amount}` | Bill amount or payment amount | (Formatted amount) |
| `{due_date}` | Bill due date | 15 Feb 2026 |
| `{period}` | Billing period | Jan 2026 |
| `{balance}` | Outstanding balance | (Formatted balance) |
...
```
Dear {customer_name}, your water bill for {period} is {amount}.
Pay via [Payment Method], Acc: {account_number}.
...
| **Phone number format** | Ensure phone numbers include the country code (for example, +1234567890). |
